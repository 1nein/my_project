...
- **`/api/` 아래 요청은 캐시하지 않는다.** 지난 AI 답변이 다시 나오면 안 된다.
  위반 판정: `public/sw.js`에서 `/api/` 경로를 캐시에 넣으면 위반이다.
- **화면은 네트워크를 먼저 본다.** 캐시를 먼저 보면 새로 배포해도 옛 화면이 계속 뜬다.
- **`public/sw.js`를 고치면 파일 안의 캐시 이름도 함께 올린다.** 이름이 그대로면 옛 캐시가
  남는다.
- 서비스 워커는 배포된 환경에서만 켠다. 개발 중에 켜면 고친 코드가 화면에 반영되지 않는다.
