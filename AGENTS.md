...
3D 인체 모델에서 아픈 부위를 눌러 증상을 기록하고, AI와 대화해 증상과 예측 병명을 정리해 남기는
웹 앱이다. 고등학교 입학 제출용 포트폴리오로, 개발자가 아닌 면접관이 열어 보고 판단한다. 혼자
만드는 개인 프로젝트이며 사용자 계정도 로그인도 없다.

Next.js(App Router) · React · TypeScript · Tailwind CSS로 만들고, 3D는 React Three Fiber로
그린다. Vercel에 배포한다.
...
├── docs/
│   ├── business-rules.md                ← 도메인 규칙. 부위 25개, 면, 증상, 기록, 대화의 동작
│   ├── architecture.md                  ← 화면과 코드의 구성, 3D를 그리는 층, 대표 흐름
│   ├── contracts.md                     ← 모델 파일의 약속, 화면과 서버가 주고받는 형식, 저장 형식
│   ├── security.md                      ← API 키를 지키는 방법, 무엇을 지키지 않기로 했는지
│   ├── standards.md                     ← 지켜야 할 규칙 전체와 검증 기준
│   ├── engineering-notes.md             ← 모르면 걸리는 것들
│   ├── operations.md                    ← 설치·실행·환경변수·모델 파일·배포
│   └── tracking/
...
전체 규칙은 `docs/standards.md`에 있다. 그중 어기면 되돌리기 어려운 것이 이 다섯이다.
...
3. **부위 식별자 25개는 `public/human-body.glb` 안의 덩어리 이름과 정확히 같아야 한다.** 앱이
   이름을 새로 지으면 눌러도 맞는 부위를 찾지 못한다. 좌우는 사용자 몸 기준이며 카메라를
   돌렸다고 뒤집지 않는다.
4. **3D 모델을 불러온 직후 덩어리마다 재질을 복제한다.** 25개가 재질 하나를 공유하고 있어서,
   복제하지 않고 색을 바꾸면 온몸이 함께 바뀐다. 그리고 모델을 그리는 코드는 기록을 직접 읽거나
   쓰지 않는다.
5. **`npm run lint`와 `npm run build`가 모두 종료 코드 0이어야 작업이 끝난 것이다.** 테스트
   프레임워크는 도입하지 않는다.
...
| 건드릴 것 | 먼저 읽을 것 |
|---|---|
| 3D 모델, 부위 선택 | `docs/contracts.md`의 모델 파일 약속, `docs/engineering-notes.md`의 3D 함정 셋 |
| 앞뒤 판별, 면 | `docs/business-rules.md`의 면 규칙, `docs/tracking/findings.md` |
...
아래 넷은 **즉시 사람에게 알린다.** 고쳐 놓고 넘어가지 않는다.
...
- 저장된 기록이 사라지거나 덮어써지는 동작을 발견했을 때. 기록은 브라우저에만 있어 복구할 수단이
  없다
- 모델 교체나 부위 목록 변경으로 **이미 저장된 기록의 부위 식별자가 맞지 않게 될 때**. 조용히
  건너뛰면 사용자는 기록이 사라졌다고 느낀다
